...
table_name = "bookings"
cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name} (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), dName  VARCHAR(255), room VARCHAR(255),phone VARCHAR(255),gender VARCHAR(255),address VARCHAR(255), image BLOB, appDate  DATETIME DEFAULT CURRENT_TIMESTAMP(), time VARCHAR(255)  , bDate DATETIME DEFAULT CURRENT_TIMESTAMP())")
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                if k%256==32:
                    ...
                    cursor.execute(f"INSERT INTO {table_name} (name,dName,room, phone,gender, address, image, appDate,time) VALUES (%s, %s,%s,  %s,%s,%s,  %s,%s,%s)", (name,doctor,room,phone,gender,address, open(image_path, "rb").read(),appDate,time))
